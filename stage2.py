...
class Transition_Object:
    def __init__(self, og_ip, og_port, modified_port):
        self.og_ip = og_ip
...
def handle_internal(data, external_ip, transtion_table, ports, port_table):
    ...
    if source_key in transtion_table:
        ...
        transition_entry = Transition_Object(source_ip, source_port, port)
...
def main():
    ...
    while True:
        ...
        # Send and Recieve
        data_bytes= handle_internal(data, external_ip, transition_table, ports, port_table)
        #threading.Thread(target=client_handler, args=[data, external_ip, external_s, real_next_hop_port, internal_s, iaddr, table]).start()
        ...
        exdata_bytes = handle_external(exdata, transition_table, port_table)
